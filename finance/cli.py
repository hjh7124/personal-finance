...
def cmd_costs(args: argparse.Namespace) -> int:
    ...
    if ws.budgets:
        ...
        for budget in ws.budgets:
            status = business_mod.evaluate_budget(budget, ws.cost_rows, as_of=as_of)
            project = business_mod.project_pace(
                status, ws.cost_rows, today=today, reference_month=as_of.plus(-1)
            )
            if project is not None:
                projects[budget.id] = project
        scoped = business_mod.in_month(ws.cost_rows, as_of)
        print(report.render_budget(
            paces, monthly_rate=business_mod.total(scoped), project_paces=projects
        ))
    print()
